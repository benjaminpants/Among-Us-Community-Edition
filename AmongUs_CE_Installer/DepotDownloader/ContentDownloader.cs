        ...
        static ulong GetSteam3DepotManifest( uint depotId, uint appId, string branch )
        {
            ...
            if ( branch != "Public" && node == KeyValue.Invalid )
            {
                ...
                if ( node_encrypted != KeyValue.Invalid )
                {
                    ...
                    if ( password == null )
                    {
                        string message = string.Format("Please enter the password for branch {0}: ", branch);
                        Config.BetaPassword = password = AmongUs_CE_Installer.ConsoleInput.ShowDialog(message);
                    }
