...
public class StatsPopup : MonoBehaviour
{
	...
	Strings.Add($"People revived: \t\t{StatsManager.Instance.RevivedPlayers}");
        ...
        foreach (KeyValuePair<string, uint> kvp in StatsManager.Instance.RoleWins)
        {
            ...
            {
                Strings.Add(kvp.Key + " W/L Ratio: \t[00FF00FF]" + kvp.Value + "[]/[FF0000FF]" + StatsManager.Instance.RoleLoses[kvp.Key] + "[]" + " [FFFF00FF](" + Mathf.RoundToInt(((float)kvp.Value / ((float)StatsManager.Instance.RoleLoses[kvp.Key] + (float)kvp.Value)) * 100f) + "%)[]");
                continue;
            }
            ...
            Strings.Add(role.RoleName + " W/L Ratio: \t[00FF00FF]" + kvp.Value + "[]/[FF0000FF]" + StatsManager.Instance.RoleLoses[kvp.Key] + "[]" + " [FFFF00FF](" + Mathf.RoundToInt(((float)kvp.Value / ((float)StatsManager.Instance.RoleLoses[kvp.Key] + (float)kvp.Value)) * 100f) + "%)[]");
        }
